...
#include <support/i386/include/mem/heap.h>
#include <support/i386/include/mem/pmm.h>
#include <support/i386/include/multiboot.h>
...
reserve_kernel()
{
	...
	for (frame = FRAME_NUMBER(addr); addr < (physaddr_t) &kernel_after;
	     addr += 4096, frame++) {
		frame_set(frame);
	}
}
